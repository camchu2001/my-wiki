...
The function part 5 can now we rewritten using arrowing function and conditional operator: 
```js
let age = prompt("What is your age?"); 
...
### 2. Stack Overflow
The call stack has a fixed size, depending on the implementation of the host environment, either the web browser of Node.js. If the number of execution contexts exceeds the size of the stack, a stack overflow error will occur. 
```js
// example: 
...
JavaScript is a **single-threaded** programming language, unlike Java, or C++, etc. 
→ The JavaScript engine has **only one call stack**. 

* When executing a script, the JavaScript engine executes code from top to bottom, line by line → *synchronous*. 
* To be more efficient, the JavaScript engine can **execute other tasks while waiting for another task to be completed** → *asynchronous*. To do this, JavaScript engine uses an **event loop**