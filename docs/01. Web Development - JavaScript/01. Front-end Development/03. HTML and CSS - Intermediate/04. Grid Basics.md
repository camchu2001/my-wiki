...
```
### 4. `grid-area`
The `grid-area` property is a shorthand property that allows defining the position and size of a grid item within the grid container. It can be used in two different ways:
...
- This name can then be *referenced* in the **`grid-template-areas`** property of the grid container to visually define the layout of the grid.
