...
> While `align-self` and `justify-self` can be used on elements with a normal block-level layout (e.g., `display: block`), their effects will be **limited**. In most cases, it is recommended to use them within Flexbox or Grid layouts for optimal alignment control.
## IV. Flexbox and Grid
### 1. Flexbox vs. Grid
