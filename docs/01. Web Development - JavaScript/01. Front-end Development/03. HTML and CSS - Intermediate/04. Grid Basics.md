...
### 1. Fractional Units
The most basic way to make grid items dynamic is by using **fractional units** (**`fr`**.) It is a unit used to distribute the remaining space in a grid container after allocating space to any grid tracks that have a fixed size. When multiple grid tracks are assigned `fr` values, the available space is divided according to the ratio of those `fr` values.
```css
