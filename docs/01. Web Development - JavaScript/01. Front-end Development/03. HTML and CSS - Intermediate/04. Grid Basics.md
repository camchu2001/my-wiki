...
#### 1. `auto-fit`
**`auto-fit`** is a value used with the `repeat()` function. It **creates as many grid tracks (rows or columns) as possible** while respecting the `max-content` size of the grid items, not overflowing the grid contrainer, and stretches the remaining grid tracks to fill the available space.

**Syntax**: `repeat(auto-fit, minmax(min-size, max-size))`
- `auto-fit` is used in conjunction with `minmax()` to define the size range for the grid tracks.
- The grid tracks will stretch to fill the available space, up to the `max-size` specified in `minmax()`.

<u>**Example:**</u> The grid container will create as many columns as possible, with a minimum width of `150px` and a maximum width of `1fr` (equal share of the remaining space).
```css
.grid-container {
...
**Syntax**: `repeat(auto-fill, minmax(min-size, max-size))`
- `auto-fill` is also used in conjunction with `minmax()` to define the size range for the grid tracks.
- The grid tracks will stretch to fill the available space, up to the `max-size` specified in `minmax()`.
