...
> While `align-self` and `justify-self` can be used on elements with a normal block-level layout (e.g., `display: block`), their effects will be **limited**. It is recommended to use them with Flexbox or Grid layouts.
## IV. Flexbox and Grid
### 1. Flexbox vs. Grid
