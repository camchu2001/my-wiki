...
### 1. Fractional Units
The most basic way to make grid items dynamic is by using **fractional units** (**`fr`**.) It is a unit used to distribute the remaining space in a grid container after allocating space to any grid tracks with a fixed size. When multiple grid tracks are assigned `fr` values, the available space is divided according to the ratio of those `fr` values.
```css
...
```
### 2. Minimum and Maximum Track Sizes
### 3. Dynamic Minimum and Maximum Sizes
#### 1. `minmax()`
...
```
#### 2. [[02. Intermediate CSS#5. [`clamp()`](https //developer.mozilla.org/en-US/docs/Web/CSS/clamp) | `clamp()`]]
...
The function can be used anywhere, not just within a grid container.
### 3. `resize`
