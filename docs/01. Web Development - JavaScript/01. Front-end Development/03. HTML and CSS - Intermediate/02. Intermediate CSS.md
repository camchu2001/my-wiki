...
### 2.  CSS Units
There are many different units used to define sizes in CSS. 
#### 1. Absolute units
...
→ Relying in `em` means that a particular size could change if the context changes, which is NOT the behavior we want. 
...
→ Using a relative size like `rem` to define font sizes across your website _is_ recommended.