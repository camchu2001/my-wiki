...
### 3. Pseudo-selectors
#### 1. [pseudo-classes](https://learn.shayhowe.com/advanced-html-css/complex-selectors/#pseudo-classes)
...
```
#### 2. [pseudo-elements](https://learn.shayhowe.com/advanced-html-css/complex-selectors/#pseudo-elements)
...
```
### 4. [Attribute Selectors](https://learn.shayhowe.com/advanced-html-css/complex-selectors/#attribute-selectors)
...
* Its position is determined by its **nearest positioned <u>ancestor</u>** (an ancestor with a `position` value of `relative`, `absolute`, `fixed`, or `sticky`). If there is no such positioned ancestor, it is positioned relative to the initial containing block (typically the document `<body>` or the root element). 
...
### 4. [Sticky Positioning](https://codepen.io/theanam/pen/MPLBYy)
...
> - [CSS `min()`, `max()`, `clamp()`](https://web.dev/articles/min-max-clamp)
...
### 3. [`min()`](https://developer.mozilla.org/en-US/docs/Web/CSS/min)
...
### 4. [`max()`](https://developer.mozilla.org/en-US/docs/Web/CSS/max)
...
### 5. [`clamp()`](https://developer.mozilla.org/en-US/docs/Web/CSS/clamp)
The **`clamp()`** function clamps a middle value within a range of values between a defined minimum and maximum bound. The function takes three parameters: **a minimum value, a preferred value, and a maximum allowed value.**<center></center>
```css
h1 {
...
## V. Custom Properties
> **Resources**
> - [CSS Custom Properties (variables)](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties#inheritance_of_custom_properties)
...
[Demonstration](https://codepen.io/Cam-Chu/pen/ExzgpOV)
```css
:root.dark {
...
2. The `light` value for the media query is for when a user has a light theme specified _or_ when they have no preference set.
...
* For example, Bootstrap provides a class called [`.btn`](https://getbootstrap.com/docs/4.0/components/buttons/#button-tags) that applies all the necessary styles to create a button with a consistent look and feel across the website. This can save developers time and effort compared to writing all the button styles themselves.
### 2. Preprocessors
> **Resource**
