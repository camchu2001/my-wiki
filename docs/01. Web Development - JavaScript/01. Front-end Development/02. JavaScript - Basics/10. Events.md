...
In this example: 
* `onclick` is an **HTML attribute**, an **event listener** that directly **specifies a string containing the JavaScript code** to be executed when the button is clicked. This code is typically a function call, but it can be any valid JavaScript expression. 
...
If the event comes from the user's actions, the `event.isTrusted` property is set to `true`. In case the event is generated by code, `event.isTrusted` is `false`. 
