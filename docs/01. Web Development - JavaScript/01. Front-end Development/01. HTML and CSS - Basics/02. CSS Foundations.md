...
```css
/* Selects all direct child <li> elements of <ul> elements */
ul > li {
  list-style-type: square;
}
```
#### 3. Adjacent Sibling Combinator
The **adjacent sibling combinator** is represented by the `"+"` symbol between two selectors. It allows you to select all elements that are **immediately preceded by the first element specified in the selector**, and both elements must have the same parent.
```css
/* Selects the first <p> element that comes immediately after an <h1> element */
h1 + p {
  font-weight: bold;
}
```
