...
Having a **large number of event handlers** on a page will directly **impact the performance**: 
...
```
The `Event` constructor takes two parameter: 
...
After creating an event, you can fire it on a target element using the `dispatchEvent()` method: 
```js
element.dispatchEvent(event); 
```

<u>**Example:**</u>
Manually create a `click` event and fire it on a button. Even if then end user doesn’t press the Test button, the event is still dispatched the event handler is executed as if the `click` event were generated by user actions.
```html
