...
```
### 9. `splice()` method
The method can be used to add/remove new items to an array. The method can take in three arguments: 
...
```
### 11. `slice()` method
The method slices out a piece of an array into a new array, it **creates a new array** containing the sliced elements and **does not remove any elements** from the source array. The method takes in the **starting and (up to but not included) ending index number**. 
```js
...
### 1. `indexOf()` method
The method searches an array for an element value and returns its index number position. 
```js
...
The method is the same as `indexOf()` but returns the position of the last occurrence of the specified element. 
```js
const fruits = ["Apple", "Orange", "Apple", "Mango"];
...
```
### 3. `includes()` method
This method checks if an element is present in an array. 
```js
const fruits = ["Banana", "Orange", "Apple", "Mango"];  
