...
In this example: 
* `onclick` **attribute** is the **event listener** that specifies the JavaScript code to be executed when the button is clicked. 
* `alert('Hello World')` is the **event handler**, the JavaScript code that defines the action to be taken when the button is clicked. 
```html
<!-- the HTML file -->
...
In this example: 
* `onclick` property in JavaScript is **not the same** as the `onclick` attribute in HTML. 
* It allows you to **assign event handlers for the click event**. When the element is clicked, the function assigned to the `onclick` property is executed.
```html
<!-- the HTML file -->
