...
In an event listener function defined using `addEventListener`, the event parameter is automatically passed by the browser when the event occurs as the first parameter. 
```js
